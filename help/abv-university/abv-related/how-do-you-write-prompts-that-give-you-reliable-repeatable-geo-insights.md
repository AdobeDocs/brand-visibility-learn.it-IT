---
title: Scrittura di richieste per informazioni geografiche affidabili e ripetibili
description: Tratta i prompt GEO come test case che sono realistici, specifici e ripetibili e configurali per ottenere informazioni stabili e affidabili sulla visibilità dell’intelligenza artificiale.
feature: Customer Configuration
role: Admin, Leader, User
level: Beginner
doc-type: Value Video
duration: 161
last-substantial-update: 2026-09-02T00:00:00Z
jira: KT-22505
source-git-commit: 823cfd29f7816f485995fcb924740a402993fa54
workflow-type: tm+mt
source-wordcount: '167'
ht-degree: 0%

---

# Come si scrivono i prompt per ottenere informazioni geografiche affidabili e ripetibili

Considera i prompt tracciati come test case. Questo video mostra come prompt realistici, specifici e ripetibili forniscono informazioni stabili e affidabili sulla visibilità dell’intelligenza artificiale in Adobe Brand Visibility.

## A chi serve questo video?

* Addetti al marketing e utenti che aggiungono prompt di tracciamento a Adobe Brand Visibility
* Team che desiderano misurazioni GEO coerenti e comparabili nel tempo

## Contenuto video

Questo video illustra:

* Perché un buon prompt si comporta come un test case: realistico, specifico e ripetibile
* Perché i prompt troppo ampi fanno oscillare il punteggio di visibilità tra le esecuzioni
* Come un obiettivo e un contesto chiari rendono i risultati più affidabili
* Perché tenere un registro con versione dei prompt e delle loro intenzioni migliora le attività nel tempo
* Come caricare le richieste in Configurazione cliente e perché modifiche frequenti possono influenzare i punteggi storici

>[!VIDEO](https://video.tv.adobe.com/v/3502722/?learn=on){transcript=true}